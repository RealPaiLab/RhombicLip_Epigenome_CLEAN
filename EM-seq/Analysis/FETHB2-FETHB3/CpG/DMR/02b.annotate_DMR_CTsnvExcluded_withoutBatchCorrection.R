...
logFile <- sprintf("%s/annotate_DMR_CTsnvExcluded_withoutBatchCorrection_%s.log",
                   outDir, 
                   dt
                   )

main <- function() {
  message("Annotate DSS DMRs with functional genomics data")
  message(sprintf("Output will be written under: %s\n",outDir))
  
  ranges_dmrs <- get_cpg_dmrs()

  ### ENCODE cCRE and DMR overlap ###
  ## Load ENCODE cCREs ##
  message("Intersecting DMRs with cCREs...")
  ranges_ccres <- get_encode_cres()
  ...
  simp_regions <- regions %>% 
    mutate(
      simplifiedType = sub(",CTCF-bound", "",ENCODE.cCRE.Type)
      ) %>% # simplify by removing CTCF-bound term
    group_by(DMR.seqnames, DMR.start, DMR.end, 
             DMR.width, DMR.strand, DMR.nCG, DMR.diff.Methy) %>% 
    summarise(
  ...
  write.table(regions,
              file=sprintf("%s/DMRs_cCRE_overlap_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  
  write.table(simp_regions,
              file=sprintf("%s/DMRs_cCRE_overlap_simplified_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  rm(ol,qry,sbj)
  
  ### Transposable elements and DMR overlap ###
  message("Intersecting DMRs with TEs...")
  
  ## Load TE db ##
  rmskhg38 <- get_repeatmasker_tes()
  ...
  write.table(TEregions,
              file=sprintf("%s/DMRs_TE_overlap_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  
  write.table(simp_TEregions,
              file=sprintf("%s/DMRs_TE_overlap_simplified_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  rm(ol,qry,sbj)
  ...
  keep <- unique(
    simp_TEregions$TE.repFamily[duplicated(simp_TEregions$TE.repFamily)]
    ) # drop those only appeared once
  p <- ggplot(data = simp_TEregions[simp_TEregions$TE.repFamily %in% keep,], 
              aes(x=TE.repFamily)
              ) +
    geom_bar() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  ...
  outFile <- sprintf("%s/DMRs.ENCODE.PLS.nearestTSS_%s.txt",outDir, dt)
  ...
  outFile <- sprintf("%s/DMRs.ENCODE.ELS.nearestTSS_%s.txt",outDir, dt)
  write.table(els,file=outFile,sep="\t",col=T,row=F,quote=F)
  
  
  ### Overlap DMR with Genes ###
  geneGR_all_body <- get_gencode_anno(region = "gene_body")
  ol <- findOverlaps(
    ranges_dmrs, geneGR_all_body, minoverlap = 0
  )
  uq_query <- length(unique(queryHits(ol)))
  uq_sbj <- length(unique(subjectHits(ol)))
  ...
  sbj <- as.data.frame(geneGR_all_body[subjectHits(ol)])
  ...
  write.table(GENEregions,
              file=sprintf("%s/DMRs_Gene_overlap_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  
  write.table(simp_GENEregions,
              file=sprintf("%s/DMRs_Gene_overlap_simplified_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  rm(ol,qry,sbj)
  ...
  ## Load feCB_CRE db ##
  feCB_CRE_Grange <- get_fetalcb_cres()
  ...
  write.table(feCBregions,
              file=sprintf("%s/DMRs_feCB_overlap_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  
  write.table(simp_feCBregions,
              file=sprintf("%s/DMRs_feCB_overlap_simplified_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  rm(ol,qry,sbj)
  ...
  write.table(simp_merged,
              file=sprintf("%s/DMRs_combined_overlap_simplified_%s.txt",outDir,dt),
              sep="\t",col=T,row=F,quote=F)
  
  ## Plot to show label connections (Sankey plot) ##
  ...
  tmp <- simp_merged %>% 
    mutate(
      id = paste(DMR.seqnames, DMR.start, DMR.end, sep = "_"), 
      DNAm = ifelse(DMR.diff.Methy > 0, "RL-SVZ_hypoDNAm", "RL-VZ_hypoDNAm")
      ) %>%
    dplyr::select(c("id", "DNAm", "ENCODE.cCRE.Type", 
             "feCB_CRE.type", "GENE.name", "TE.repClass")) %>%
    mutate(ENCODE.cCRE.Type = 
             ifelse(ENCODE.cCRE.Type %in% ENCODE.cCRE.Type_keep, 
                    paste("ENCODE.cCRE.Type", ENCODE.cCRE.Type, sep = "_"), 
                    "ENCODE.cCRE.Type_Others")
           ) %>%
    mutate(feCB_CRE.type = paste("feCB_CRE.type", feCB_CRE.type, sep = "_")) %>%
    mutate(GENE.name = 
             ifelse(GENE.name != "noMatch", 
                    "GENE.name.Gene", 
                    paste("GENE.name", GENE.name, sep = "_"))
           ) %>%
    mutate(TE.repClass = ifelse(TE.repClass %in% TEclass_keep,
                                paste("TE.repClass", TE.repClass, sep = "_"),
                                "TE.repClass_Others")
           )
  
  links_count <- NULL
  links_count <- rbind(links_count,
                       as.data.frame(table(tmp$TE.repClass, tmp$ENCODE.cCRE.Type))
                       )
  links_count <- rbind(links_count,
                       as.data.frame(table(tmp$ENCODE.cCRE.Type, tmp$feCB_CRE.type))
                       )
  links_count <- rbind(links_count,
                       as.data.frame(table(tmp$feCB_CRE.type, tmp$DNAm))
                       )
  links_count <- rbind(links_count,
                       as.data.frame(table(tmp$DNAm, tmp$GENE.name))
                       )
  ...
  # plot sankey #
  sankey <- sankeyNetwork(Links = links_count, Nodes = nodes, 
                          Source = "IDsource", Target = "IDtarget", 
                          Value = "value", NodeID = "name", 
                          sinksRight=FALSE, fontSize=16, 
                          nodeWidth=50, nodePadding=0
                          )
  htmlwidgets::saveWidget(sankey, 
                          sprintf(
                            "%s/DMRs_combined_overlap_simplified_SankeyPlot_%s.html",
                            outDir,
                            dt),
                          selfcontained = TRUE)
}
