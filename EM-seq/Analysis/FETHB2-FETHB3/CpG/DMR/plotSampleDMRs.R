...
###cat("Plotting top 5 promoter DMRs\n")
###prom <- prom[order(abs(prom$diff.Methy), decreasing=TRUE),]
###source("locusPlotter.R")
###for (g in prom$tss[1:5]) { #,"ATOH7","FOXJ1","BRINP1")) {
###print(g)
###x <- prom[which(prom$tss == g)[1],]
###print(x[,c("name","diff.Methy")])
###  methyPlotter(
###    bsObj$bs, 
###    region = x, 
###    extend = 2000,
...
###geneList <- c("SOX2","WLS",DEG$gene[1:20])
###tmp <- subset(DEG, gene %in% geneList)  
#### plot a volcano plot with labelled points for top 20 genes
###p <- ggplot(DEG, aes(x = logFC, y = -log10(PValue))) +
###  geom_point(aes(color = FDR < 0.05 & abs(logFC) > 0.3), alpha = 0.5) +
###  scale_color_manual(values = c("grey", "red")) +
###  labs(title = "Volcano plot of RNA-seq DEGs: VZ vs SVZ",
###
...
###  show.legend = FALSE
###)
###tmp2 <- subset(DEG, gene %in% prom$tss)
###p <- p + geom_point(
###  data = tmp2,
###  aes(x = logFC, y = -log10(PValue)),
###  color = "green",
...
###)
###
###tmp2 <- subset(DEG, gene %in% c(geneList, prom$tss))
###p <- p + geom_text_repel(
###  data = tmp2,
###  aes(label = gene),
###  size = 3,
...
###)
###ggsave(
###  filename = sprintf("%s/DEG_VZ_vs_SVZ_volcano.pdf", outDir),
###  plot = p,
###  width = 8, height = 6
###)
...
###)
###
#### plot average diffMethy for each logFC decile
###deciles <- cut(sig$logFC, 
###  breaks = quantile(sig$logFC, probs = seq(0, 1, 0.25)),
###  include.lowest = TRUE
###)
###sig$decile <- deciles
###avg_diffMethy <- sig %>%
###  group_by(decile) %>%
###  summarise(avg_diffMethy = median(diffMethy, na.rm = TRUE))
###p <- ggplot(avg_diffMethy, aes(x = decile, y = avg_diffMethy * 100)) +
###  geom_bar(stat = "identity", fill = "steelblue") +
###  labs(title = "Median Diff Methylation",
###       x = "Deciles of Log2FC",
###       y = "Average Diff Methylation (%)") +
###  theme_minimal(base_size = 12) +
###  scale_x_discrete(labels = function(x) gsub("Decile", "", x))
###ggsave(
###  filename = sprintf("%s/AllPromoters_diffMethy_by_Log2FC_decile.pdf",
###    outDir),
###  plot = p,
###  width = 8, height = 6
###)
###
#### now plot boxplot of DNAm for each decile of logFC
###p <- ggplot(sig, aes(x = decile, y = diffMethy * 100, fill = decile)) +
###  geom_boxplot(outlier.shape = NA) +
###  geom_jitter(width = 0.2, alpha = 0.5) +
###  labs(title = "Boxplot of Diff Methylation by Log2FC Decile",
###       x = "Deciles of Log2FC",
###       y = "Diff Methylation (%)") +
###  theme_minimal(base_size = 12) +
...
###ggsave(
###  filename = sprintf("%s/AllPromoters_diffMethy_boxplot_by_Log2FC_decile.pdf",
###    outDir),
###  plot = p,
###  width = 8, height = 6
###)
...
for (i in c(6)) { # ,2)) { #1:length(geneList)) {
  g <- geneList[i]
  print(g)
...
dmrs$diffMethy100 <- dmrs$diff.Methy * 100
p <- ggplot(dmrs, aes(x = diffMethy100)) + # fill = status)) +
  geom_histogram(colour="white", fill="grey20",binwidth=5) +
...
cat("Plot DMR locations on all chromosomes\n")
...
#plotDMRsOnChromosomes(dmrGR,outDir)
