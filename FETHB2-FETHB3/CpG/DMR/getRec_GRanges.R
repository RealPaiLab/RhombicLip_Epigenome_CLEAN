...
getRec_GRanges <- function(tbxFile, gr, context="CG",unit="pctM",
	STR_POS=3,CONTEXT_POS=6,C_POS=4,T_POS=5,numFields=7,splitStrands=FALSE,
    printRecords=FALSE, base_minCvg=1L,numCores=1L,verbose=FALSE
) {

hdr <-c("chr","pos","strand","C_count","T_count","context","context2")
...
out <- mclapply(res, function(m) {
	...
	if (length(idx)<1) emptyFlag <- TRUE
		...
		rec[,C_POS] <- as.integer(as.character(rec[,C_POS]))
		rec[,T_POS] <- as.integer(as.character(rec[,T_POS]))
		colnames(rec) <- hdr
	}
}
...
if (any(sno_idx)) {
	cat("this block of code hasn't been implemented and needs to be checked"); 
